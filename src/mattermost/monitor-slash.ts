  ...
  try {
    ...
    if (isLoopbackHost(callbackHost) && !isLoopbackHost(mmHost)) {
      params.runtime.error?.(
        `mattermost-pro: slash commands callbackUrl resolved to ${params.callbackUrl} (loopback) while baseUrl is ${params.baseUrl}. This MAY be unreachable depending on your deployment. If native slash commands don't work, set channels.mattermost-pro.commands.callbackUrl to a URL reachable from the Mattermost server (e.g. your public reverse proxy URL).`,
      );
    }
  } catch {
...
async function registerSlashCommandsAcrossTeams(params: {
  ...
  for (const team of params.teams) {
    try {
      ...
      params.runtime.error?.(
        `mattermost-pro: failed to register slash commands for team ${team.id}: ${String(err)}`,
      );
    }
  }
...
export async function registerMattermostMonitorSlashCommands(params: {
  ...
  try {
    ...
    if (teamRegistrationFailures > 0) {
      params.runtime.error?.(
        `mattermost-pro: slash command registration completed with ${teamRegistrationFailures} team error(s)`,
      );
    }
    ...
    params.runtime.log?.(
      `mattermost-pro: slash commands registered (${registered.length} commands across ${teams.length} teams, callback=${slashCallbackUrl})`,
    );
  } catch (err) {
