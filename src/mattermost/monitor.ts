...
export async function monitorMattermostProvider(opts: MonitorMattermostOpts = {}): Promise<void> {
  ...
  if (!botToken) {
    throw new Error(
      `Mattermost bot token missing for account "${account.accountId}" (set channels.mattermost-pro.accounts.${account.accountId}.botToken or MATTERMOST_BOT_TOKEN for default).`,
    );
  }
  const baseUrl = normalizeMattermostBaseUrl(opts.baseUrl ?? account.baseUrl);
  if (!baseUrl) {
    throw new Error(
      `Mattermost baseUrl missing for account "${account.accountId}" (set channels.mattermost-pro.accounts.${account.accountId}.baseUrl or MATTERMOST_URL for default).`,
    );
  }
  ...
  await runWithReconnect(
    ...
    },
    {
      ...
      onReconnect: (delayMs) => {
        runtime.log?.(`mattermost-pro: API not accessible, retrying in ${Math.round(delayMs / 1000)}s`);
      },
    },
  );
  ...
  try {
    ...
    if (isLoopbackHost(callbackHost) && !isLoopbackHost(mmHost)) {
      runtime.error?.(
        `mattermost-pro: interactions callbackUrl resolved to ${callbackUrl} (loopback) while baseUrl is ${baseUrl}. This MAY be unreachable depending on your deployment. If button clicks don't work, set channels.mattermost-pro.interactions.callbackBaseUrl to a URL reachable from the Mattermost server (e.g. your public reverse proxy URL).`,
      );
    }
    if (!isLoopbackHost(callbackHost) && allowedInteractionSourceIps.length === 0) {
      runtime.error?.(
        `mattermost-pro: interactions callbackUrl resolved to ${callbackUrl} without channels.mattermost-pro.interactions.allowedSourceIps. For safety, non-loopback callback sources will be rejected until you allowlist the Mattermost server or trusted ingress IPs.`,
      );
    }
  } catch {
  ...
  const unregisterInteractions = registerPluginHttpRoute({
    ...
    handler: createMattermostInteractionHandler({
      ...
      resolveSessionKey: async ({ channelId, userId, post }) => {
        ...
        if (!channelInfo?.type) {
          logVerboseMessage(
            `mattermost-pro: drop interaction session event (cannot resolve channel type for ${channelId})`,
          );
      ...
      dispatchButtonClick: async (opts) => {
        ...
        if (!channelInfo?.type) {
          logVerboseMessage(
            `mattermost-pro: drop interaction dispatch (cannot resolve channel type for ${opts.channelId})`,
          );
          return;
        }
        const kind = mapMattermostChannelTypeToChatType(channelInfo.type);
        ...
        const ctxPayload = core.channel.reply.finalizeInboundContext({
          ...
          From:
            kind === "direct"
              ? `mattermost-pro:${opts.userId}`
              : kind === "group"
                ? `mattermost-pro:group:${opts.channelId}`
                : `mattermost-pro:channel:${opts.channelId}`,
          To: to,
          ...
          Provider: "mattermost-pro" as const,
          Surface: "mattermost-pro" as const,
          ...
          OriginatingChannel: "mattermost-pro" as const,
          OriginatingTo: to,
        });
  ...
  const { groupPolicy, providerMissingFallbackApplied } =
    resolveAllowlistProviderRuntimeGroupPolicy({
      providerConfigPresent: cfg.channels?.["mattermost-pro"] !== undefined,
  ...
  const runModelPickerCommand = async (params: {
    ...
    const ctxPayload = core.channel.reply.finalizeInboundContext({
      ...
      From:
        params.kind === "direct"
          ? `mattermost-pro:${params.senderId}`
          : params.kind === "group"
            ? `mattermost-pro:group:${params.channelId}`
            : `mattermost-pro:channel:${params.channelId}`,
      To: to,
  ...
  const handlePost = async (
    ...
    const replayResult = await processMattermostReplayGuardedPost({
      ...
      handlePost: async () => {
        ...
        if (accessDecision.ingress.decision !== "allow") {
          if (kind === "direct") {
            ...
            if (accessDecision.ingress.decision === "pairing") {
              ...
              logVerboseMessage(
                `mattermost-pro: pairing request sender=${senderId} created=${created}`,
              );
              if (created) {
                try {
                  ...
                  logVerboseMessage(
                    `mattermost-pro: pairing reply failed for ${senderId}: ${String(err)}`,
                  );
                }
              }
              return;
          ...
          logVerboseMessage(
            `mattermost-pro: drop group message (groupPolicy=${groupPolicy} reason=${accessDecision.senderAccess.reasonCode})`,
          );
          return;
        }
        ...
        if (mentionDecision.dropReason === "onchar-not-triggered") {
          logVerboseMessage(
            `mattermost-pro: drop group message (onchar not triggered channel=${channelId} sender=${senderId})`,
          );
        ...
        if (mentionDecision.dropReason === "missing-mention") {
          logVerboseMessage(
            `mattermost-pro: drop group message (missing mention channel=${channelId} sender=${senderId} requireMention=${shouldRequireMention} bypass=${shouldBypassMention} canDetectMention=${canDetectMention})`,
          );
          recordPendingHistory();
          return;
        }
        ...
        if (!bodyText) {
          logVerboseMessage(
            `mattermost-pro: drop group message (empty body after normalization channel=${channelId} sender=${senderId})`,
          );
          return;
        }
        ...
        const ctxPayload = core.channel.reply.finalizeInboundContext({
          ...
          From:
            kind === "direct"
              ? `mattermost-pro:${senderId}`
              : kind === "group"
                ? `mattermost-pro:group:${channelId}`
                : `mattermost-pro:channel:${channelId}`,
          To: to,
            ...
            adapter: {
              ...
              resolveTurn: () => ({
                ...
                record: {
                            ...
                            inboundLastRouteSessionKey === route.mainSessionKey && pinnedMainDmOwner
                              ? {
                                  ...
                                  onSkip: ({
                                    ...
                                    logVerboseMessage(
                                      `mattermost-pro: skip main-session last route for ${senderRecipient} (pinned owner ${ownerRecipient})`,
                                    );
                                  },
                                }
                              : undefined,
                  ...
                  onRecordError: (err) => {
                    logVerboseMessage(
                      `mattermost-pro: failed updating session meta id=${post.id ?? "unknown"}: ${String(err)}`,
                    );
                  },
                },
                history: {
    ...
    if (replayResult === "duplicate") {
      logVerboseMessage(
        `mattermost-pro: drop post (dedupe account=${account.accountId} ids=${allMessageIds.length})`,
      );
      return;
    }
  };
  ...
  const handleReactionEvent = async (payload: MattermostEventPayload) => {
    ...
    if (!channelId) {
      ...
      logVerboseMessage(
        `mattermost-pro: drop reaction (no channel_id in broadcast, cannot enforce policy)`,
      );
      return;
    }
    const channelInfo = await resolveChannelInfo(channelId);
    ...
    if (reactionAccess.ingress.decision !== "allow") {
      if (kind === "direct") {
        logVerboseMessage(
          `mattermost-pro: drop reaction (dmPolicy=${dmPolicy} sender=${userId} reason=${reactionAccess.senderAccess.reasonCode})`,
        );
      } else {
        logVerboseMessage(
          `mattermost-pro: drop reaction (groupPolicy=${groupPolicy} sender=${userId} reason=${reactionAccess.senderAccess.reasonCode} channel=${channelId})`,
        );
      }
      return;
    }
