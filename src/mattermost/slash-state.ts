...
export function activateSlashCommands(params: {
  ...
  log?.(
    `mattermost-pro: slash commands activated for account ${accountId} (${registeredCommands.length} commands)`,
  );
}
...
export function registerSlashCommandRoute(api: OpenClawPluginApi) {
  ...
  const routeHandler = async (req: IncomingMessage, res: ServerResponse) => {
    ...
    if (match.kind === "ambiguous") {
      api.logger.warn?.(
        `mattermost-pro: slash callback matched multiple accounts via ${match.source} (${match.accountIds.join(", ")})`,
      );
      const conflictText =
        match.source === "token"
