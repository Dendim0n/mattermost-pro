...
export async function registerSlashCommands(params: {
  ...
  for (const spec of commands) {
    ...
    if (ownedCommands.length === 0 && foreignCommands.length > 0) {
      log?.(
        `mattermost-pro: trigger /${spec.trigger} already used by non-OpenClaw command(s); skipping to avoid mutating external integrations`,
      );
      continue;
    }
    ...
    if (ownedCommands.length > 1) {
      log?.(
        `mattermost-pro: multiple owned commands found for /${spec.trigger}; using the first and leaving extras untouched`,
      );
    }
    ...
    if (existingCmd && existingNeedsUpdate) {
      log?.(
        `mattermost-pro: command /${spec.trigger} exists with different callback settings; updating (id=${existingCmd.id})`,
      );
      try {
        ...
        log?.(
          `mattermost-pro: failed to update command /${spec.trigger} (id=${existingCmd.id}): ${String(err)}`,
        );
        ...
        try {
          ...
          log?.(
            `mattermost-pro: failed to delete stale command /${spec.trigger} (id=${existingCmd.id}): ${String(deleteErr)}`,
          );
