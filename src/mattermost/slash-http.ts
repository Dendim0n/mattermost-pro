...
async function fetchCurrentMattermostCommandUncached(params: {
  ...
  try {
    ...
    if (commandLookupError) {
      params.log?.(
        `mattermost-pro: slash command lookup by id failed for /${sanitizeMattermostLogValue(params.registered.trigger)}; using team list fallback: ${sanitizeCommandLookupError(commandLookupError)}`,
      );
    } else if (commandLookupFallbackDetail) {
      params.log?.(
        `mattermost-pro: slash ${commandLookupFallbackDetail} for /${sanitizeMattermostLogValue(params.registered.trigger)}; using team list fallback`,
      );
    }
    ...
    const primaryDetail = commandLookupError
      ...
      : commandLookupFallbackDetail
        ...
        : "";
    params.log?.(
      `mattermost-pro: slash command registration check failed for /${sanitizeMattermostLogValue(params.registered.trigger)}: ${sanitizeCommandLookupError(err)}${primaryDetail}`,
    );
    return null;
  }
}
  ...
  if (!commandLookupInflight.has(lookupKey)) {
    ...
    if (!reservation.allowed) {
      if (reservation.shouldLog) {
        params.log?.(
          `mattermost-pro: slash command validation lookup rate-limited for /${sanitizeMattermostLogValue(params.registeredCommand.trigger)}`,
        );
      }
      return false;
    }
...
async function authorizeSlashInvocation(params: {
  ...
  try {
    ...
    log?.(
      `mattermost-pro: slash channel lookup failed for ${sanitizeMattermostLogValue(channelId)}: ${sanitizeCommandLookupError(err)}`,
    );
  }
...
export function createSlashCommandHttpHandler(params: SlashHttpHandlerParams) {
  ...
  return async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    log?.(
      `mattermost-pro: slash command /${sanitizeMattermostLogValue(trigger)} from ${sanitizeMattermostLogValue(senderName)} in ${sanitizeMattermostLogValue(channelId)}`,
    );
...
async function handleSlashCommandAsync(params: {
  ...
  const textLimit = core.channel.text.resolveTextChunkLimit(cfg, "mattermost-pro", account.accountId, {
