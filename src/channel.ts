...
>[0]): ChannelMessageToolDiscovery {
  ...
  const actionsConfig = cfg.channels?.["mattermost-pro"]?.actions as { reactions?: boolean } | undefined;
...
const mattermostMessageActions: ChannelMessageActionAdapter = {
  ...
  handleAction: async ({ action, params, cfg, accountId }) => {
    if (action === "react") {
      ...
      const mattermostConfig = cfg.channels?.["mattermost-pro"] as MattermostConfig | undefined;
...
const mattermostOutbound: ChannelOutboundAdapter = {
  ...
  sendPayload: async (ctx) => {
    ...
    if (buttons?.length) {
      ...
      return attachChannelToResult("mattermost-pro", result);
    }
    return await sendTextMediaPayload({ channel: "mattermost-pro", ctx, adapter: mattermostOutbound });
  },
  resolveTarget: ({ to }) => {
...
export const mattermostProPlugin: ChannelPlugin<ResolvedMattermostAccount> = createChatChannelPlugin({
  base: {
    id: "mattermost-pro",
    meta: {
      ...meta,
  ...
  pairing: {
    text: {
      ...
      notify: createLoggedPairingApprovalNotifier(
        ({ id }) => `[mattermost-pro] User ${id} approved for pairing`,
      ),
    },
  },
  threading: {
