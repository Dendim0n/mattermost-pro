import { _ as normalizeMattermostBaseUrl, a as resolveMattermostReplyToMode, b as updateMattermostPost, d as fetchMattermostChannel, g as fetchMattermostUserTeams, i as resolveMattermostAccount, l as createMattermostPost, m as fetchMattermostUser, n as listMattermostAccountIds, o as MattermostPostSchema, p as fetchMattermostMe, s as createMattermostClient, t as MATTERMOST_TEXT_CHUNK_LIMIT, u as deleteMattermostPost, v as readMattermostError, y as sendMattermostTyping } from "./constants-lvhO5Lit.mjs";
import { t as getMattermostProRuntime } from "./runtime-CtaUqSvR.mjs";
import { S as resolveChannelMediaMaxBytes, T as warnMissingProviderGroupPolicyFallbackOnce, _ as logTypingFailure, a as resolveMattermostMonitorInboundAccess, b as registerPluginHttpRoute, c as buildModelsProviderData, d as createChannelPairingController, g as logInboundDrop, l as createChannelHistoryWindow, m as listSkillCommandsForAgents, o as DEFAULT_GROUP_HISTORY_LIMIT, r as normalizeMattermostAllowEntry, s as buildAgentMediaPayload, t as authorizeMattermostCommandInvocation, u as createChannelMessageReplyPipeline, v as parseStrictPositiveInteger, w as resolveDefaultGroupPolicy, x as resolveAllowlistProviderRuntimeGroupPolicy } from "./monitor-auth-BAOA0ZPg.mjs";
import { C as resolveCallbackUrl, S as registerSlashCommands, _ as renderMattermostProviderPickerView, a as sendMessageMattermost, b as cleanupSlashCommands, c as computeInteractionCallbackUrl, d as setInteractionCallbackUrl, f as setInteractionSecret, g as renderMattermostModelsPickerView, h as parseMattermostModelPickerContext, l as createMattermostInteractionHandler, m as buildMattermostAllowedModelRefs, n as deactivateSlashCommands, o as resolveMattermostOpaqueTarget, p as deliverMattermostReplyPayload, r as getSlashCommandState, s as buildButtonProps, t as activateSlashCommands, u as resolveInteractionCallbackPath, v as resolveMattermostModelPickerCurrentModel, w as resolveSlashCommandConfig, x as isSlashCommandsEnabled, y as DEFAULT_COMMAND_SPECS } from "./slash-state-BRtyJ5Mi.mjs";
import { defineFinalizableLivePreviewAdapter, deliverWithFinalizableLivePreviewAdapter } from "openclaw/plugin-sdk/channel-message";
...
//#region src/mattermost/directory.ts
...
//#endregion
//#region src/mattermost/draft-stream.ts
...
function createMattermostDraftStream(params) {
	const maxChars = Math.min(params.maxChars ?? 14e3, MATTERMOST_TEXT_CHUNK_LIMIT);
...
//#endregion
//#region src/mattermost/monitor-gating.ts
...
//#endregion
//#region src/mattermost/monitor-helpers.ts
...
//#endregion
//#region src/mattermost/monitor-onchar.ts
...
//#endregion
//#region src/mattermost/monitor-resources.ts
...
function createMattermostMonitorResources(params) {
	...
	const resolveMattermostMedia = async (fileIds) => {
		...
		for (const fileId of ids) try {
			...
			logger.debug?.(`mattermost-pro: failed to download file ${fileId}: ${String(err)}`);
		}
		return out;
	};
	...
	const resolveChannelInfo = async (channelId) => {
		...
		try {
			...
			logger.debug?.(`mattermost-pro: channel lookup failed: ${String(err)}`);
			channelCache.set(channelId, {
				value: null,
				expiresAt: Date.now() + CHANNEL_CACHE_TTL_MS
			});
	...
	const resolveUserInfo = async (userId) => {
		...
		try {
			...
			logger.debug?.(`mattermost-pro: user lookup failed: ${String(err)}`);
			userCache.set(userId, {
				value: null,
				expiresAt: Date.now() + USER_CACHE_TTL_MS
			});
...
//#endregion
//#region src/mattermost/monitor-slash.ts
function buildSlashCommands(params) {
	...
	try {
		...
		params.runtime.error?.(`mattermost-pro: failed to list skill commands: ${String(err)}`);
	}
...
function warnOnSuspiciousCallbackUrl(params) {
	try {
		...
		if (isLoopbackHost(callbackHost) && !isLoopbackHost(mmHost)) params.runtime.error?.(`mattermost-pro: slash commands callbackUrl resolved to ${params.callbackUrl} (loopback) while baseUrl is ${params.baseUrl}. This MAY be unreachable depending on your deployment. If native slash commands don't work, set channels.mattermost-pro.commands.callbackUrl to a URL reachable from the Mattermost server (e.g. your public reverse proxy URL).`);
	} catch {}
}
async function registerSlashCommandsAcrossTeams(params) {
	...
	for (const team of params.teams) try {
		...
		params.runtime.error?.(`mattermost-pro: failed to register slash commands for team ${team.id}: ${String(err)}`);
	}
	return {
		registered,
...
async function registerMattermostMonitorSlashCommands(params) {
	...
	try {
		...
		if (teamRegistrationFailures > 0) params.runtime.error?.(`mattermost-pro: slash command registration completed with ${teamRegistrationFailures} team error(s)`);
		...
		params.runtime.log?.(`mattermost-pro: slash commands registered (${registered.length} commands across ${teams.length} teams, callback=${slashCallbackUrl})`);
	} catch (err) {
		params.runtime.error?.(`mattermost-pro: failed to register slash commands: ${String(err)}`);
	}
}
//#endregion
//#region src/mattermost/monitor-websocket.ts
				...
				const runHealthCheck = async () => {
					...
					try {
						...
						if (current !== initialUpdateAt) {
							opts.runtime.log?.(`mattermost-pro: bot account updated (update_at changed: ${initialUpdateAt} → ${current}) — reconnecting`);
							stopHealthChecks();
							ws.terminate();
						}
					} catch (err) {
...
//#endregion
//#region src/mattermost/no-visible-reply-diagnostic.ts
/**
...
//#endregion
//#region src/mattermost/reconnect.ts
/**
...
//#endregion
//#region src/mattermost/monitor.ts
...
function shouldUseMattermostBlockDraftPreview(account) {
	return account.streamingMode !== "off" && (account.streamingMode === "block" || account.blockStreaming === true);
}
...
async function deliverMattermostReplyWithDraftPreview(params) {
	...
	const deliverNormally = async (payload) => {
...
async function monitorMattermostProvider(opts = {}) {
	const core = getMattermostProRuntime();
	...
	if (!botToken) throw new Error(`Mattermost bot token missing for account "${account.accountId}" (set channels.mattermost-pro.accounts.${account.accountId}.botToken or MATTERMOST_BOT_TOKEN for default).`);
	const baseUrl = normalizeMattermostBaseUrl(opts.baseUrl ?? account.baseUrl);
	if (!baseUrl) throw new Error(`Mattermost baseUrl missing for account "${account.accountId}" (set channels.mattermost-pro.accounts.${account.accountId}.baseUrl or MATTERMOST_URL for default).`);
	...
	await runWithReconnect(async () => {
		...
		onError: (err) => {
			runtime.error?.(`mattermost-pro: API auth failed: ${String(err)}`);
			opts.statusSink?.({
		...
		onReconnect: (delayMs) => {
			runtime.log?.(`mattermost-pro: API not accessible, retrying in ${Math.round(delayMs / 1e3)}s`);
		}
	});
	if (opts.abortSignal?.aborted) return;
	...
	try {
		...
		if (isLoopbackHost(callbackHost) && !isLoopbackHost(mmHost)) runtime.error?.(`mattermost-pro: interactions callbackUrl resolved to ${callbackUrl} (loopback) while baseUrl is ${baseUrl}. This MAY be unreachable depending on your deployment. If button clicks don't work, set channels.mattermost-pro.interactions.callbackBaseUrl to a URL reachable from the Mattermost server (e.g. your public reverse proxy URL).`);
		if (!isLoopbackHost(callbackHost) && allowedInteractionSourceIps.length === 0) runtime.error?.(`mattermost-pro: interactions callbackUrl resolved to ${callbackUrl} without channels.mattermost-pro.interactions.allowedSourceIps. For safety, non-loopback callback sources will be rejected until you allowlist the Mattermost server or trusted ingress IPs.`);
	} catch {}
	...
	const unregisterInteractions = registerPluginHttpRoute({
		...
		handler: createMattermostInteractionHandler({
			...
			resolveSessionKey: async ({ channelId, userId, post }) => {
				...
				if (!channelInfo?.type) {
					logVerboseMessage(`mattermost-pro: drop interaction session event (cannot resolve channel type for ${channelId})`);
			...
			dispatchButtonClick: async (opts) => {
				...
				if (!channelInfo?.type) {
					logVerboseMessage(`mattermost-pro: drop interaction dispatch (cannot resolve channel type for ${opts.channelId})`);
					return;
				}
				const kind = mapMattermostChannelTypeToChatType(channelInfo.type);
				...
				const ctxPayload = core.channel.reply.finalizeInboundContext({
					...
					OriginatingChannel: "mattermost-pro",
					OriginatingTo: to
				});
				const textLimit = core.channel.text.resolveTextChunkLimit(cfg, "mattermost-pro", account.accountId, { fallbackLimit: account.textChunkLimit ?? 14e3 });
	...
	const logger = core.logging.getChildLogger({ module: "mattermost-pro" });
	...
	const { groupPolicy, providerMissingFallbackApplied } = resolveAllowlistProviderRuntimeGroupPolicy({
		providerConfigPresent: cfg.channels?.["mattermost-pro"] !== void 0,
		groupPolicy: account.config.groupPolicy,
		defaultGroupPolicy
	});
	warnMissingProviderGroupPolicyFallbackOnce({
		providerMissingFallbackApplied,
		providerKey: "mattermost-pro",
		accountId: account.accountId,
	...
	const runModelPickerCommand = async (params) => {
		...
		const ctxPayload = core.channel.reply.finalizeInboundContext({
			...
			From: params.kind === "direct" ? `mattermost-pro:${params.senderId}` : params.kind === "group" ? `mattermost-pro:group:${params.channelId}` : `mattermost-pro:channel:${params.channelId}`,
			To: to,
		...
		if (await processMattermostReplayGuardedPost({
			...
			handlePost: async () => {
				...
				if (senderId === botUserId) {
					logVerboseMessage(`mattermost-pro: drop post (self sender=${senderId})`);
					return;
				}
				if (isSystemPost(post)) {
					logVerboseMessage(`mattermost-pro: drop post (system post type=${post.type ?? "unknown"})`);
					return;
				}
				const channelInfo = await resolveChannelInfo(channelId);
				const channelType = normalizeOptionalString(channelInfo?.type) ?? normalizeOptionalString(payload.data?.channel_type);
				if (!channelType) {
					logVerboseMessage(`mattermost-pro: drop post (cannot resolve channel type for ${channelId})`);
					return;
				}
				...
				if (accessDecision.ingress.decision !== "allow") {
					if (kind === "direct") {
						if (accessDecision.ingress.reasonCode === "dm_policy_disabled") {
							logVerboseMessage(`mattermost-pro: drop dm (dmPolicy=disabled sender=${senderId})`);
							return;
						}
						if (accessDecision.ingress.decision === "pairing") {
							...
							logVerboseMessage(`mattermost-pro: pairing request sender=${senderId} created=${created}`);
							if (created) try {
								...
								logVerboseMessage(`mattermost-pro: pairing reply failed for ${senderId}: ${String(err)}`);
							}
							return;
						}
						logVerboseMessage(`mattermost-pro: drop dm sender=${senderId} (dmPolicy=${dmPolicy})`);
						return;
					}
					...
					if (accessDecision.ingress.reasonCode === "group_policy_not_allowlisted") {
						logVerboseMessage(`mattermost-pro: drop group sender=${senderId} (not in groupAllowFrom)`);
						return;
					}
					logVerboseMessage(`mattermost-pro: drop group message (groupPolicy=${groupPolicy} reason=${accessDecision.senderAccess.reasonCode})`);
					return;
				}
				...
				if (mentionDecision.dropReason === "onchar-not-triggered") {
					logVerboseMessage(`mattermost-pro: drop group message (onchar not triggered channel=${channelId} sender=${senderId})`);
					recordPendingHistory();
					return;
				}
				if (mentionDecision.dropReason === "missing-mention") {
					logVerboseMessage(`mattermost-pro: drop group message (missing mention channel=${channelId} sender=${senderId} requireMention=${shouldRequireMention} bypass=${shouldBypassMention} canDetectMention=${canDetectMention})`);
					recordPendingHistory();
					return;
				}
				...
				if (!bodyText) {
					logVerboseMessage(`mattermost-pro: drop group message (empty body after normalization channel=${channelId} sender=${senderId})`);
					return;
				}
				...
				const ctxPayload = core.channel.reply.finalizeInboundContext({
					...
					From: kind === "direct" ? `mattermost-pro:${senderId}` : kind === "group" ? `mattermost-pro:group:${channelId}` : `mattermost-pro:channel:${channelId}`,
					To: to,
				...
				const resolvePreviewFinalText = (text) => {
					...
					const chunkMode = core.channel.text.resolveChunkMode(cfg, "mattermost-pro", account.accountId);
				...
				const appendBlockDraftEntry = (text) => {
					const cleaned = text?.trim();
					if (!cleaned || cleaned === lastBlockDraftEntry) return;
									...
									updateLastRoute: kind === "direct" ? {
										...
										mainDmOwnerPin: inboundLastRouteSessionKey === route.mainSessionKey && pinnedMainDmOwner ? {
											...
											onSkip: ({ ownerRecipient, senderRecipient }) => {
												logVerboseMessage(`mattermost-pro: skip main-session last route for ${senderRecipient} (pinned owner ${ownerRecipient})`);
											}
										} : void 0
									} : void 0,
									onRecordError: (err) => {
										logVerboseMessage(`mattermost-pro: failed updating session meta id=${post.id ?? "unknown"}: ${String(err)}`);
									}
								},
								history: {
									...
									run: () => core.channel.reply.dispatchReplyFromConfig({
										...
										replyOptions: {
											...
											onPartialReply: (payload) => {
												if (!useBlockDraftPreview && shouldUpdateMattermostDraftFromAssistantPartial(account.streamingMode)) updateDraftFromPartial(payload.text);
											},
											...
											onReasoningStream: async () => {
												if (!lastPartialText) if (useBlockDraftPreview) appendBlockDraftEntry("Thinking…");
												else draftStream.update("Thinking…");
											},
											onToolStart: async (payload) => {
												...
												if (useBlockDraftPreview) appendBlockDraftEntry(statusText);
												else draftStream.update(statusText);
											},
											onItemEvent: async (payload) => {
												...
												if (progressText) if (useBlockDraftPreview) appendBlockDraftEntry(progressText);
												else draftStream.update(progressText);
											}
										}
									})
								})
			...
			logVerboseMessage(`mattermost-pro: drop post (dedupe account=${account.accountId} ids=${allMessageIds.length})`);
			return;
		}
	};
	const handleReactionEvent = async (payload) => {
		...
		if (!channelId) {
			logVerboseMessage(`mattermost-pro: drop reaction (no channel_id in broadcast, cannot enforce policy)`);
			return;
		}
		const channelInfo = await resolveChannelInfo(channelId);
		if (!channelInfo?.type) {
			logVerboseMessage(`mattermost-pro: drop reaction (cannot resolve channel type for ${channelId})`);
			return;
		}
		const kind = mapMattermostChannelTypeToChatType(channelInfo.type);
		...
		if (reactionAccess.ingress.decision !== "allow") {
			if (kind === "direct") logVerboseMessage(`mattermost-pro: drop reaction (dmPolicy=${dmPolicy} sender=${userId} reason=${reactionAccess.senderAccess.reasonCode})`);
			else logVerboseMessage(`mattermost-pro: drop reaction (groupPolicy=${groupPolicy} sender=${userId} reason=${reactionAccess.senderAccess.reasonCode} channel=${channelId})`);
			return;
		}
		const teamId = channelInfo?.team_id ?? void 0;
		...
		core.system.enqueueSystemEvent(eventText, {
			sessionKey,
			contextKey: `mattermost-pro:reaction:${postId}:${emojiName}:${userId}:${action}`
		});
	...
	const debouncer = core.channel.debounce.createInboundDebouncer({
		...
		buildKey: (entry) => {
			...
			return `mattermost-pro:${account.accountId}:${channelId}:${threadKey}`;
		},
	...
	if (slashEnabled) {
		const runAbortCleanup = () => {
			...
			slashShutdownCleanup = cleanupSlashCommands({
				...
				runtime.error?.(`mattermost-pro: slash cleanup failed: ${String(err)}`);
			});
		};
...
//#endregion
//#region src/mattermost/probe.ts
...
//#endregion
//#region src/mattermost/reactions.ts
