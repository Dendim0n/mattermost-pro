...
async function registerSlashCommands(params) {
	...
	try {
		...
		log?.(`mattermost-pro: failed to list existing commands: ${String(err)}`);
		throw err;
	}
	...
	for (const spec of commands) {
		...
		if (ownedCommands.length === 0 && foreignCommands.length > 0) {
			log?.(`mattermost-pro: trigger /${spec.trigger} already used by non-OpenClaw command(s); skipping to avoid mutating external integrations`);
			continue;
		}
		if (ownedCommands.length > 1) log?.(`mattermost-pro: multiple owned commands found for /${spec.trigger}; using the first and leaving extras untouched`);
		...
		if (existingCmd && !existingNeedsUpdate) {
			log?.(`mattermost-pro: command /${spec.trigger} already registered (id=${existingCmd.id})`);
			registered.push({
				id: existingCmd.id,
		...
		if (existingCmd && existingNeedsUpdate) {
			log?.(`mattermost-pro: command /${spec.trigger} exists with different callback settings; updating (id=${existingCmd.id})`);
			try {
				...
				log?.(`mattermost-pro: failed to update command /${spec.trigger} (id=${existingCmd.id}): ${String(err)}`);
				try {
					await deleteMattermostCommand(client, existingCmd.id);
					log?.(`mattermost-pro: deleted stale command /${spec.trigger} (id=${existingCmd.id})`);
				} catch (deleteErr) {
					log?.(`mattermost-pro: failed to delete stale command /${spec.trigger} (id=${existingCmd.id}): ${String(deleteErr)}`);
					continue;
				}
			}
		}
		try {
			...
			log?.(`mattermost-pro: registered command /${spec.trigger} (id=${created.id})`);
			registered.push({
				id: created.id,
				trigger: spec.trigger,
				teamId,
			...
			log?.(`mattermost-pro: failed to register command /${spec.trigger}: ${String(err)}`);
		}
	}
	return registered;
}
...
async function cleanupSlashCommands(params) {
	...
	for (const cmd of commands) {
		...
		try {
			...
			log?.(`mattermost-pro: deleted command /${cmd.trigger} (id=${cmd.id})`);
		} catch (err) {
			log?.(`mattermost-pro: failed to delete command /${cmd.trigger}: ${String(err)}`);
		}
	}
}
/**
...
//#endregion
//#region src/mattermost/model-picker.ts
...
//#endregion
//#region src/mattermost/reply-delivery.ts
async function deliverMattermostReplyPayload(params) {
	...
	const chunkMode = params.core.channel.text.resolveChunkMode(params.cfg, "mattermost-pro", params.accountId);
...
//#endregion
//#region src/mattermost/interactions.ts
	...
	return async (req, res) => {
		...
		try {
			...
			core.system.enqueueSystemEvent(eventLabel, {
				sessionKey,
				contextKey: `mattermost-pro:interaction:${payload.post_id}:${actionId}`
			});
		} catch (err) {
...
//#endregion
//#region src/mattermost/target-resolution.ts
...
const getCore = () => getMattermostProRuntime();
...
async function resolveMattermostSendContext(to, opts) {
	...
	if (!token) throw new Error(`Mattermost bot token missing for account "${account.accountId}" (set channels.mattermost-pro.accounts.${account.accountId}.botToken or MATTERMOST_BOT_TOKEN for default).`);
...
//#endregion
//#region src/mattermost/slash-http.ts
...
async function fetchCurrentMattermostCommandUncached(params) {
	...
	try {
		...
		if (commandLookupError) params.log?.(`mattermost-pro: slash command lookup by id failed for /${sanitizeMattermostLogValue(params.registered.trigger)}; using team list fallback: ${sanitizeCommandLookupError(commandLookupError)}`);
		else if (commandLookupFallbackDetail) params.log?.(`mattermost-pro: slash ${commandLookupFallbackDetail} for /${sanitizeMattermostLogValue(params.registered.trigger)}; using team list fallback`);
		...
		params.log?.(`mattermost-pro: slash command registration check failed for /${sanitizeMattermostLogValue(params.registered.trigger)}: ${sanitizeCommandLookupError(err)}${primaryDetail}`);
		return null;
	}
}
...
async function validateMattermostSlashCommandToken(params) {
	...
	if (!commandLookupInflight.has(lookupKey)) {
		...
		if (!reservation.allowed) {
			if (reservation.shouldLog) params.log?.(`mattermost-pro: slash command validation lookup rate-limited for /${sanitizeMattermostLogValue(params.registeredCommand.trigger)}`);
			return false;
		}
	}
...
async function authorizeSlashInvocation(params) {
	...
	try {
		...
		log?.(`mattermost-pro: slash channel lookup failed for ${sanitizeMattermostLogValue(channelId)}: ${sanitizeCommandLookupError(err)}`);
	}
...
function createSlashCommandHttpHandler(params) {
	...
	return async (req, res) => {
		...
		log?.(`mattermost-pro: slash command /${sanitizeMattermostLogValue(trigger)} from ${sanitizeMattermostLogValue(senderName)} in ${sanitizeMattermostLogValue(channelId)}`);
		...
		try {
			...
			log?.(`mattermost-pro: slash command handler error: ${sanitizeCommandLookupError(err)}`);
			try {
...
//#endregion
//#region src/mattermost/slash-state.ts
...
function activateSlashCommands(params) {
	...
	log?.(`mattermost-pro: slash commands activated for account ${accountId} (${registeredCommands.length} commands)`);
}
/**
...
function registerSlashCommandRoute(api) {
	...
	const routeHandler = async (req, res) => {
		...
		if (match.kind === "ambiguous") {
			api.logger.warn?.(`mattermost-pro: slash callback matched multiple accounts via ${match.source} (${match.accountIds.join(", ")})`);
